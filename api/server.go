...
func (s *ApiServer) listen() {
	r := mux.NewRouter()
	r.HandleFunc("/health", s.HealthIndex)
	r.HandleFunc("/stats", s.StatsIndex)
	r.HandleFunc("/live_stats", s.StatsIndex)
	r.HandleFunc("/stats_address", s.MinersIndex)
	r.HandleFunc("/get_blocks", s.BlocksIndex)
	r.HandleFunc("/get_payments", s.PaymentsIndex)
	r.HandleFunc("/get_payment", s.PaymentsIndex)
	r.HandleFunc("/stats_address/{login:0x[0-9a-fA-F]{40}}", s.AccountIndex)
